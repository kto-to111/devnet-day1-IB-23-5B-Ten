...
def dump_json_deterministic(obj: Any, path: str) -> str:
...
def fetch_online(url: str, timeout: int = 10) -> Tuple[int, Dict[str, Any]]:
...
def build_summary(student_token: str, student_name: str, student_group: str,
                  ...
                  validation_errors: List[str], response_sha256: str) -> Dict[str, Any]:
    return {
...
def main() -> int:
    ...
    # Получаем данные студента из окружения
    ...
    try:
        if args.offline:
            if not os.path.exists(RESPONSE_PATH):
                print(f"ERROR: {RESPONSE_PATH} not found for offline mode.", file=sys.stderr)
                return 1
            payload = load_json(RESPONSE_PATH)
            status_code = 200  # имитация успеха для кеша
            log("OFFLINE mode: loaded cached response.json")
        else:
            log(f"ONLINE mode: GET {url}")
            status_code, payload = fetch_online(url)
            # Сохраняем детерминированно сразу после получения
            dump_json_deterministic(payload, RESPONSE_PATH)

        ok, errors = validate_payload(payload)

        # Хэш считаем строго от сохранённого "детерминированного" текста файла
        ...
        # Вывод результата в консоль для проверки студентом
...
if __name__ == "__main__":
    # Вызов main и корректный выход с кодом возврата
    sys.exit(main())